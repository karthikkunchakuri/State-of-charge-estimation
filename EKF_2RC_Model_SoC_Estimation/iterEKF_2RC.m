function [zkbar,zkbarbnd,dzk,dzkbnd,ekfData] = iterEKF_2RC(vk,ik,Tk,deltat,ekfData)
model = ekfData.model;
...
yhat = OCVfromSOCtemp(xhat(zkInd),Tk,model) - R1*xhat(irInd1) - R2*xhat(irInd2) - R0*ik;
...
offset       = - R1*xhat(irInd1) - R2*xhat(irInd2) - R0*(ik);
