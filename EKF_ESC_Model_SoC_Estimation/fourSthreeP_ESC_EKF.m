...
clear; clc; close  
load P14_ESCmodel;                                                         % ESC model of Panasonic cell
udds = load('udds.txt');                                                   % profile of current versus time for UDDS
T    = 25;

z0_1 = 0.7:-0.1:0.4;                                                         % set initial SOC for each cell in pack
z0_2 = 0.71:-0.1:0.41;
z0_3 = 0.72:-0.1:0.42;
z0   = [z0_1 z0_2 z0_3];
R0_1 = (2.5:-0.1:2.1)*1e-3;                                                  % set R0 for each cell in pack
R0_2 = (2.5:-0.1:2.1)*1e-3;
R0_3 = (2.5:-0.1:2.1)*1e-3;
R0   = [R0_1 R0_2 R0_3];
Q0_1 = 14:17;                                                                % set Q for each cell in pack
Q0_2 = 14.2:17.2;
Q0_3 = 14.3:17.3;
Q0   = [Q0_1 Q0_2 Q0_3];
...
load P14_ESCmodel.mat; % loads cell model
load P14_ESC_PackData.mat;
...
SigmaX0 = diag([1e2 1e2 1e-4 1e-2]); % uncertainty of initial state
SigmaV = 1e-3; % uncertainty of voltage sensor, output equation
SigmaW = 1e-1; % uncertainty of current sensor, bias

ekfData = initEKF_ESC(voltage(1,:), T, SigmaX0, SigmaV, SigmaW, model);
...
for k = 1:size(voltage,1)
  ...
  [sochat(k),socbound(k),dsochat(k,:),dsocbound(k,:),ekfData] = ...
      iterEKF_ESC(vk,ik,Tk,deltat,ekfData);
...
subplot(2,2,3);
sochat = repmat(sochat,1,12);
socbound = repmat(socbound,1,12);
plot(time/60,100*(sochat+dsochat), '-r'); hold on
h = plot([time/60; NaN; time/60],...
        [100*(sochat+dsochat+socbound+dsocbound); NaN(1,12); 100*(sochat+dsochat-dsocbound-socbound)]);
plot(time/60,100*soc, '-k');
ylim([35 80]);
