...
title('Voltage versus time for 4S2P cells');

subplot(1,2,2);
plot(t,100*zk); xlabel('Time (hr)'); ylabel('State of charge (%)');
title('SOC versus time for 4S2P cells');
...
subplot(2,2,3);
sochat = repmat(sochat,1,8);
socbound = repmat(socbound,1,8);
plot(time/60,100*(sochat+dsochat), '-r'); hold on
h = plot([time/60; NaN; time/60],...
        [100*(sochat+dsochat+socbound+dsocbound); NaN(1,8); 100*(sochat+dsochat-dsocbound-socbound)]);
plot(time/60,100*soc, '-k');
ylim([35 80]);
