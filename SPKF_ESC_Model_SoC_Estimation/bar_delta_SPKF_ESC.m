...
R0 = (2.6:-0.1:2.1)*1e-3;                                                  % set R0 for each cell in pack
...
for k = 1:length(z0)
  ...
  [vcell,rck,zcell,OCV] = simCell_RC(ik,T,1,model,z0(k),0);
  vk(:,k) = vcell;
  zk(:,k) = zcell;
end
...
% estimates this bias
ibias = 0.015;                                                               % can set this (e.g., to 0 or 0.5 A)
...
SigmaX0 = diag([1e2 1e2 1e-4 1e-2 5e-2]);                                  % uncertainty of initial state
SigmaV = 1e-3;                                                             % uncertainty of voltage sensor, output equation
SigmaW = diag([1e-1, 1e-4]);                                               % uncertainty of current sensor, bias

spkfData = initSPKF_ESC(voltage(1,:), T, SigmaX0, SigmaV, SigmaW, model);
...
for k = 1:size(voltage,1)
  ...
  [sochat(k), socbound(k), dsochat(k,:), dsocbound(k,:), bias(k), biasBound(k), spkfData] = ...
      iterSPKF_ESC(vk, ik, Tk, deltat, spkfData);
