function [zkbar, zkbarbnd, dzk, dzkbnd, ib, ibbnd, spkfData] = ...
  ...
  Wc = spkfData.Wc;
  irInd1 = spkfData.irInd1;
  irInd2 = spkfData.irInd2;
  ...
  offset = M*xhat(hkInd) - R1*xhat(irInd1) - R2*xhat(irInd2) - R0*(ik-ib);
  ...
  function xnew = stateEqn(xold, current, xnoise)
    ...
    xnew = 0*xold;
    xnew(irInd1,:) = RC1*xold(irInd1,:) + (1-RC1)*(current-xold(ibInd,:));
    xnew(irInd2,:) = RC2*xold(irInd2,:) + (1-RC2)*(current-xold(ibInd,:));
  ...
  function yhat = outputEqn(xhat, current, ynoise, T, model)
    yhat = OCVfromSOCtemp(xhat(zkInd,:), T, model) + M*xhat(hkInd,:);
    yhat = yhat - R1*xhat(irInd1,:) - R2*xhat(irInd2,:) - R0*(current - xhat(ibInd,:)) + ynoise(1,:);
  end
